...
class MessageProducer {
    ...
    sizeSum() {
        return this._adapter.sizeSum();
    }

    maxMemorySize() {
        return this._adapter.maxMemorySize();
    }

    getLostMessages(nodeName) {
        return this._adapter.getLostMessages(nodeName);
    }

    get statistics() {
        const statisticsPerNode = [];
        this._consumers.forEach((nodeName) => {
            const queueSize = this._adapter.queueSize(nodeName);
            const sent = this._adapter.sent(nodeName);
            const singleNodeStatistics = {
                ...
                sent,
                queueSize,
                dropped: this._adapter.getLostMessages(nodeName)
            };
            statisticsPerNode.push(singleNodeStatistics);
        });
        const statistics = {
            binarySize: this._adapter.messageQueue.sizeSum,
            configuredMaxBinarySize: this._adapter.maxMemorySize,
            statisticsPerNode
        };
        return statistics;
    }
