...
const sleepWhile = require('../../../../utils/sleepWhile');
...
class ZMQProducer {
    ...
    start() {
        this._backend.on('message', (...args) => {
            ...
            const workerReady = args[1];
            const consumerType = this._encoding.decode(args[2], { customEncode: false });

            if (![this._PPP_READY.toString('utf8'), this._PPP_HEARTBEAT.toString('utf8')].includes(workerReady.toString('utf8'))) {
    ...
    async close(force = true) {
        await sleepWhile(() => this._messageQueue.queue.length > 0 && !force);
