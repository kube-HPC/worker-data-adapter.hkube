...
class ZMQProducer {
    ...
    get sizeSum() {
        return this._messageQueue.sizeSum;
    }
    ...
    getLostMessages(consumerType) {
        return this._messageQueue.lostMessages(consumerType);
    }
    ...
    async close(force = true) {
        if (!force) {
            await waitFor(() => {
                const queueSize = this._messageQueue.queue.length;
                if (queueSize > 0) {
                    log.throttle.info(`producer queue size during close is ${queueSize}`);
                }
                return queueSize === 0;
            });
        }
