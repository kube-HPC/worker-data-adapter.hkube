
const EventEmitter = require('events');
const objectPath = require('object-path');
const { Encoding } = require('@hkube/encoding');
const consts = require('../consts/messages').server;
const { Server } = require('./adapters/zeroMQ/server');


class DataServer extends Encoding {
    constructor({ port, encoding = 'json' }) {
        super({ type: encoding });
        this.emitter = new EventEmitter();
        this.server = new Server({ port });
        setImmediate(() => this.init());
        this.endSendingState();
    }

    on(...rest) {
        this.emitter.on(...rest);
    }

    setSendingState(taskId, data) {
        this.taskId = taskId;
        this.data = data;
    }

    endSendingState() {
        this.taskId = null;
        this.data = null;
    }

    init() {
        this.serving = 0;
        this.server.on('bind', (data) => {
            this.emitter.emit('bind', data);
        });
        this.server.on('message', async (request) => {
            this.serving += 1;
            try {
                const parsedReq = await this.decode(request);
                const { taskId, dataPath } = parsedReq;
                if (this.taskId !== taskId) {
                    this._send({ message: consts.notAvailable, reason: `Current taskId is ${this.taskId}` });
                    return;
                }
                let toSend = this.data;
                if (dataPath) {
                    toSend = objectPath.get(toSend, dataPath);
                }
                if (toSend === undefined) {
                    this._send({ error: consts.noSuchDataPath, reason: `${dataPath} does not exist in data` });
                    return;
                }
                this._send({ message: consts.success, data: toSend });
            }
            finally {
                this.serving -= 1;
            }
        });
    }

    isServing() {
        return this.serving > 0;
    }

    _send(objToSend) {
        this.server.send(this.encode(objToSend));
    }

    close() {
        this.server.close();
    }
}
module.exports = DataServer;
