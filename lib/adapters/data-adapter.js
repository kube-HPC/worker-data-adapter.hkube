...
class DataAdapter extends EventEmitter {
    async init(options, dataServer) {
        ...
        this._batchRequest = timing(this._batchRequest.bind(this));
        this._getFromPeer = timing(this._getFromPeer.bind(this));
        this._getFromCache = timing(this._getFromCache.bind(this));
        this._getFromStorage = timing(this._getFromStorage.bind(this));
    }
    ...
    encodeHeaderPayload(value) {
        return this._encoding.encodeHeaderPayload(value);
    }

    decodeHeaderPayload(header, value) {
        return this._encoding.decodeHeaderPayload(header, value);
    }
    ...
    async setData(options, tracer) {
        ...
        const result = await storageManager.storage.put({ path, header, data: newData, encodeOptions: { ignoreEncode: true } }, tracer);
        return result;
    }
    ...
    async _getFromPeerInternal(options) {
        ...
        if (this._dataServer && host === this._dataServer._host && port === this._dataServer._port) {
            ...
            responses = dataList.map(d => ({ size: d.value.length, content: this.decodeHeaderPayload(d.header, d.value) }));
        }
        else {
    ...
    async _getFromStorage(options, trace) {
        const { size, payload } = await storageManager.storage.getCustomData(options, trace);
        return { size, response: payload };
    }
}
